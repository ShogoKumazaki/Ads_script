function main() {
  ...
  function createReportQuery(level, startDate, endDate) {
    ...
    if (level === 'customer') {
      selectClause = "SELECT segments.date, customer.id, metrics.cost_micros, metrics.clicks, metrics.conversions, metrics.impressions, metrics.search_impression_share ";
    } else if (level === 'campaign') {
      selectClause = "SELECT segments.date, " + level + ".name, metrics.cost_micros, metrics.clicks, metrics.conversions, metrics.impressions, metrics.search_impression_share, metrics.search_top_impression_share, metrics.search_absolute_top_impression_share, metrics.search_rank_lost_impression_share, metrics.search_rank_lost_top_impression_share, metrics.search_rank_lost_absolute_top_impression_share, metrics.search_click_share";
    } else {
      selectClause = "SELECT segments.date, " + level + ".name, metrics.cost_micros, metrics.clicks, metrics.conversions, metrics.impressions, metrics.search_impression_share, metrics.search_top_impression_share, metrics.search_absolute_top_impression_share, metrics.search_rank_lost_impression_share, metrics.search_rank_lost_top_impression_share, metrics.search_rank_lost_absolute_top_impression_share";
    }
  ...
  function setJapaneseHeaders(sheet, level) {
    const headers = level === 'customer' 
      ? ["日付", "顧客ID", "費用 (円)", "クリック数", "コンバージョン数", "インプレッション数", "IS"] 
      : ["日付", "名前", "費用 (円)", "クリック数", "コンバージョン数", "インプレッション数", "IS", "上部IS", "最上部IS", "IS損失率(ランク)", "上位IS損失率(ランク)", "最上位IS損失率(ランク)", "クリックシェア"];
